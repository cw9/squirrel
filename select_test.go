...
func TestSelectBuilderPlaceholders(t *testing.T) {
	b := Select("test").PreWhere("px = ? AND py = ?").Where("x = ? AND y = ?")

	sql, _, _ := b.PlaceholderFormat(Question).ToSql()
	assert.Equal(t, "SELECT test PREWHERE px = ? AND py = ? WHERE x = ? AND y = ?", sql)

	sql, _, _ = b.PlaceholderFormat(Dollar).ToSql()
	assert.Equal(t, "SELECT test PREWHERE px = $1 AND py = $2 WHERE x = $3 AND y = $4", sql)

	sql, _, _ = b.PlaceholderFormat(Colon).ToSql()
  assert.Equal(t, "SELECT test PREWHERE px = :1 AND py = :2 WHERE x = :3 AND y = :4", sql)

	sql, _, _ = b.PlaceholderFormat(AtP).ToSql()
	assert.Equal(t, "SELECT test PREWHERE px = @p1 AND py = @p2 WHERE x = @p3 AND y = @p4", sql)
}
